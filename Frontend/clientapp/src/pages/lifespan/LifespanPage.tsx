...
export default function LifespanPage() {
  ...
  const [maxActionsInput, setMaxActionsInput] = useState(
    selectedComponent?.maxActions || 0
  );
  ...
  function findSelectedComponent(components: Component[]) {
    if (state && state.componentId && components) {
      setSelectedComponent(
        components.filter((c) => c.id == state.componentId)[0]
      );
    }
  }
      ...
      {selectedComponent && (
        ...
        >
          ...
          <div className="Planner">
            ...
            >
              <textarea
                placeholder={`${t("maintenance.label")} ${t(
                  "description.label"
                )}... `}
                value={description}
