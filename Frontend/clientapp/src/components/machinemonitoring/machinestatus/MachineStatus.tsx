...
export default function MachineStatus(props: IMachineStatus) {
    useEffect(() => {
        // let graph = document.querySelector(`#${props.name}`)?.innerHTML
        // if (graph) { graph = '' }
        drawUptime()
    }, [props.uptime]);
    ...
    function drawUptime() {
        if (props.uptime && props.uptime.length) {
            ...
            const xDiff = new Date(props.uptime[props.uptime.length - 1].end).getTime() / 1000 - new Date(props.uptime[0].begin).getTime() / 1000
            const scale = svgWidth / xDiff

            const svg = d3.select(`#${props.name}`)
                .append('svg')
                .attr('height', '2rem')
                .attr('width', '100%')
                .attr('margin', 0)
            ...
            svg.selectAll('mybar')
                ...
                .attr('width', u => { return calcDifference(u.begin, u.end) * scale })
                .attr('height', 32)
