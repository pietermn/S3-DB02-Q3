...
export default function ActionsGraph(props: IActionsGraph) {
  ...
  ref = useD3(
    (svg: d3.Selection<SVGElement, {}, HTMLElement, any>) => {
      ...
      const yAxis = (g: any) => {
        g.attr("transform", `translate(${margin.left + 20},-10)`).call(
          d3.axisLeft(y)
        );
      };
  ...
  useEffect(() => {
    async function AsyncGetActions() {
      setIsLoading(true);
      setActions(
        await GetPreviousActions(props.componentId, beginDate, endDate)
      );
      setIsLoading(false);
    }
    AsyncGetActions();
  ...
  return (
    <div className="actionsgraph-container">
      ...
      <CircularProgress
        className={isLoading ? "loading" : "loading invisible"}
      />
    </div>
  );
}
