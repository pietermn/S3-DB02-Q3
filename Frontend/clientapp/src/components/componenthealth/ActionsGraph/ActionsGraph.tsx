...
export default function ActionsGraph(props: IActionsGraph) {
    ...
    const [myWidth] = useState(((window.innerWidth / 3) * 2) * .75);
    const [myHeight] = useState(((window.innerHeight / 2)) * .60);
    ...
    function DrawGraph() {
        ...
        const x = d3.scaleBand()
            .range([0, myWidth])
            .domain(actions.slice(0).reverse().map((d, i) => `${timespan.slice(0, -1)} ${i + 1}`))
            .padding(0.2);
