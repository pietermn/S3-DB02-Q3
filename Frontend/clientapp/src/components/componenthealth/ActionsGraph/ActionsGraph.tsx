...
export default function ActionsGraph(props: IActionsGraph) {
    ...
    ref = useD3(
        (svg: d3.Selection<SVGElement, {}, HTMLElement, any>) => {
            ...
            const x = d3
                ...
                .domain(
                    actions
                        .slice(0)
                        .reverse()
                        .map((d) => `${d.timespanIndicator} ${d.currentTimespan}`)
                )
            ...
            svg.select(".plot-area")
                ...
                .duration(800)
                .attr("fill", "#69b3a2")
                .attr("x", (d, i) => x(`${d.timespanIndicator} ${d.currentTimespan}`) || 0)
    ...
    useEffect(() => {
        async function AsyncGetActions() {
            setIsLoading(true);
            setActions(await (await GetPreviousActions(props.componentId, beginDate, endDate)).reverse());
            setIsLoading(false);
        }
        AsyncGetActions();
    ...
    async function handleInput(e: any) {
        ...
        switch (e.target.name) {
            case "timespan":
                ...
                setActions(await GetPreviousActions(props.componentId, e.target.value, amountTimespan));
                break;
            case "timespan-amount":
                setAmountTimespan(e.target.value);
                setActions(await GetPreviousActions(props.componentId, timespan, e.target.value));
                break;
        }
        setIsLoading(false);
    }
